...
if [ -f "frontend/package.json" ]; then
  ...
  cd frontend
  command -v pnpm &>/dev/null || { echo "Installing pnpm..."; npm install -g pnpm 2>/dev/null || echo "WARN: pnpm install failed"; }
  [ -d "node_modules" ] || { echo "Installing frontend dependencies..."; pnpm install 2>/dev/null || echo "WARN: pnpm install failed"; }
